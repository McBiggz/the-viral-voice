...
function updateCount(){if(count)count.textContent=`${Math.min(found.length,6)} / 6`;document.documentElement.style.setProperty('--signal',`${Math.min(100,10+found.reduce((n,id)=>n+(eggs[id]?.gain||0),0))}%`)}
...
function openEgg(id,button){const egg=eggs[id];if(!egg)return;const fresh=!found.includes(id);if(fresh){found.push(id);localStorage.setItem(EGG_KEY,JSON.stringify(found));button?.classList.add('egg-found');showToast(`SIGNAL RECOVERED // ${String(found.length).padStart(2,'0')}`)}title.textContent=egg.title;copy.textContent=egg.copy;strength.textContent=fresh?`+${egg.gain}%`:'RECOVERED';modal.hidden=false;document.body.style.overflow='hidden';updateCount()}
...
window.addEventListener('viralvoice:signalchange',e=>{const backend=Number(e.detail?.strength)||10,local=found.reduce((n,id)=>n+(eggs[id]?.gain||0),0);document.documentElement.style.setProperty('--signal',`${Math.min(100,backend+local)}%`)});
updateCount();
})();